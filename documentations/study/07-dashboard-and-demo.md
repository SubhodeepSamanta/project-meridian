...
The lifecycle track turns the platform into a story: observe the system, decide an action, contain compromise, and recover with a new credential. Its active state is derived from API evidence, so a running incident changes the story rather than triggering a fake animation. An open incident's status is also an intentional control: clicking `OPEN ↗` expands the response details in place; clicking `Begin verified recovery ↗` calls the recovery endpoint and renders `Recovering incident…` before the card becomes resolved.
...
- The rail uses a small navigation state machine: `getInitialNavSection()` reads the hash, the click handler updates state immediately, and a passive scroll listener compares each section's document position against a fixed reading marker. `aria-current="page"` mirrors that state. A separate mobile effect uses the nav element's `scrollTo()` and link offsets to center the active tab without moving the document itself.
- The shell's `overflow-x-hidden` utility was removed from `<main>` and replaced with `body { overflow-x: hidden; }`. This keeps one authoritative vertical scroll owner, so native hash navigation and the `window.scrollY` scroll spy agree. The important lesson is to isolate horizontal overflow without accidentally creating a second vertical scrolling context.
- The active-section marker is viewport-aware: `clamp(viewport height × 0.4, 180px, 420px)` in equivalent code terms. A fixed 150px marker made a large screen feel wrong because a visible Evidence panel could remain labeled Incidents until its top reached an arbitrary point. The rail guide repeats the meaning of the current step in plain language, so the interface teaches the showcase path while it is being used.
- The last readability pass separates size from contrast. The `--muted` and `--faint` tokens are now darker on light mode and brighter on dark mode, while headings, statuses, event names, actors, trace details, and interactive labels use stronger weights. This fixes text that was technically present but visually washed out or too thin.
...
The contrast review found a similar problem in both themes: some copy used a deliberately quiet color but became difficult to scan at the actual phone viewport. The fix is token-level first—stronger `--muted`, `--faint`, and border values—followed by targeted font-weight overrides. This keeps the quiet editorial hierarchy while ensuring a label such as `LAST SERVER EVENT`, `ACTION APPROVED`, or `POST /audit/integrity` reads immediately. The same review found that the incident `OPEN` status looked clickable but was only a span; it is now a real disclosure button with an expanded response panel, and the recovery button has a visible busy label and a verified post-action state.
