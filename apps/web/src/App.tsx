...
function StatusPill({ value, onClick, ariaLabel, expanded, disabled }: { value: string; onClick?: () => void; ariaLabel?: string; expanded?: boolean; disabled?: boolean }) {
  const content = <><i />{value.replaceAll("_", " ")}{onClick && <span className="status-pill-cue" aria-hidden="true">↗</span>}</>;
  if (!onClick) return <span className={`status-pill ${tone(value)}`} role="status">{content}</span>;
  return <button className={`status-pill status-pill-button ${tone(value)}`} type="button" onClick={onClick} aria-label={ariaLabel ?? `${value.replaceAll("_", " ")} status`} aria-expanded={expanded} disabled={disabled} title="View incident response details">{content}</button>;
}
      ...
      <section className="content-shell">
        ...
        <div className="page-content">
          ...
          <section className="lower-grid" id="incidents">
             <div className="panel incidents-panel"><div className="panel-heading"><div><SectionEyebrow number="05" label="ISOLATE THE UNKNOWN" /><h2>When trust breaks.</h2></div><span className="panel-count">{snapshot.incidents.length.toString().padStart(2, "0")} stories</span></div><div className="incident-list">{snapshot.incidents.length === 0 && <div className="empty-state">No incidents. The quiet is earned by observable controls.</div>}{displayedIncidents.map((incident) => { const owner = snapshot.identities.find((identity) => identity.id === incident.identity_id); const stages = incident.status === "open" ? ["presented", "unknown", "quarantined", "revoked"] : ["detected", "quarantined", "revoked", "recovered"]; const activeStage = incident.status === "open" ? 2 : 3; const incidentExpanded = expandedIncidentId === incident.id; return <div className={`incident-card ${incident.status}`} key={incident.id}><div className="incident-spine"><span>{incident.status === "open" ? "↯" : "✓"}</span></div><div className="incident-content"><div className="incident-top"><strong>{incident.reason}</strong>{incident.status === "open" ? <StatusPill value={incident.status} onClick={() => setExpandedIncidentId((current) => current === incident.id ? null : incident.id)} expanded={incidentExpanded} disabled={busy !== null} ariaLabel={`Open incident details for ${owner?.name ?? shortId(incident.identity_id)}`} /> : <StatusPill value={incident.status} />}</div><p>{owner?.name ?? shortId(incident.identity_id)} <span>·</span> opened {formatAge(incident.created_at)}</p><div className="isolation-badge"><i />{incident.status === "open" ? "UNKNOWN CREDENTIAL ISOLATED" : "IDENTITY RESTORED WITH PROOF"}</div>{incidentExpanded && <div className="incident-details" role="status"><span>RESPONSE DETAILS</span><strong>Credential blocked before service access.</strong><p>Linked identity: {owner?.name ?? shortId(incident.identity_id)}. Use the verified recovery action to issue a replacement credential and close this incident with evidence.</p></div>}<div className="containment-track">{stages.map((stage, index) => <span className={index <= activeStage ? "done" : ""} key={stage}><i />{stage}</span>)}</div>{incident.status === "open" ? <button className="inline-action" disabled={busy !== null} onClick={() => void perform(`recover:${incident.id}`, () => recoverIncident(incident.id), "Replacement credential issued; identity recovered.")}>{busy === `recover:${incident.id}` ? "Recovering incident…" : "Begin verified recovery"} <span>↗</span></button> : <div className="resolved-copy">Recovered {formatDate(incident.resolved_at)} <span>· evidence complete</span></div>}</div></div>; })}</div></div>
